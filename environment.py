...
# ── Event definitions ─────────────────────────────────────────────────────
# Each entry: (severity, category, pipeline_id, event_type)
...
# Events that require evidence before a fix action is valid
...
# Actions that are never correct for performance / design-debt events
WRONG_FOR_PERFORMANCE = {"restart_job", "rollback_deployment"}

# Minimum evidence score required before escalating
EVIDENCE_THRESHOLD = 0.5

# Hard step limits — prevents infinite loops, enforces ~20-min runtime
...
class PipelineWatchdogEnv(Environment):
    ...
    Agent receives pipeline alert observations and must choose the correct
    remediation action. Evidence must be gathered before escalating complex
    issues — going to stakeholders without proof triggers a penalty.

    Reward range: [-1, +1] per step.
    Grading: resolved / total - missed_penalty + evidence_bonus.
    """
    ...
    def reset(self, task_id: str = "easy") -> WatchdogObservation:
        ...
        # Preserve previous episode id for cross-run comparison
        ...
        # Shuffle so agent cannot memorise a fixed event order
    ...
    def step(self, action: WatchdogAction) -> WatchdogObservation:
        ...
        # inspect_logs, classify_layer, compare_runs are always free.
        # analyze_query is free only when the current event is NOT SLOW_QUERY
        # (for SLOW_QUERY it IS the correct fix action — goes to _evaluate_fix).
        ...
        # Advance to the next queued event if the current one was resolved
        ...
        # Restore earned reward — _next_event_obs always resets it to 0.0
    ...
    def _evaluate_fix(self, action: WatchdogAction) -> WatchdogObservation:
        """
        Evaluate a fix action against the current active event.
        Returns a reward observation; clears _current_event on success.
        """
        ...
        # ── Default: unrecognised or mismatched action ────────────────────
    ...
    def _handle_diagnostic(self, action: WatchdogAction) -> WatchdogObservation:
        """
        Handle free diagnostic actions: inspect_logs, classify_layer,
        compare_runs, analyze_query (when not the fix for SLOW_QUERY).
        These never consume an event but do increment evidence_steps.
        """
        act   = action.action.lower().strip()
        ...
        if act == "inspect_logs":
            ...
            return WatchdogObservation(
                success=True,
                message=(
                    f"🔍 Inspect logs: {pname}\n"
                    f"{log_str}\n"
                    f"   Unresolved: {self._bitlog.count_unresolved(pid)}"
                ),
        ...
        if act == "classify_layer":
            ...
            verdict = (
                "SCRIPT/SQL layer" if script_errs >= api_errs
                else "API/NETWORK layer"
            )
        ...
        # ── analyze_query (as a free diagnostic) ─────────────────────────
        if act == "analyze_query":
    ...
    def _next_event_obs(self, prefix: str) -> WatchdogObservation:
        """
        Dequeue the next event, log it to BitLog, and return an observation.
        Sets done=True and returns a completion observation if the queue is empty.
        """
        if self._event_index >= len(self._event_queue):
            self._state.done = True
            return WatchdogObservation(
    ...
    def _wrong(self, message: str, hint: str, reward: float) -> WatchdogObservation:
        """
        Return a failed observation, always echoing back the current event
        context so the agent can re-attempt with full information.
        """
        ev  = self._current_event or {}
    ...
    def _reward_for_severity(self, sev: str) -> float:
        """Base reward scaled by event severity."""
    ...
    def grade(self, trajectory: list) -> float:
        ...
        Formula:
          ...
          evidence = min(evidence_steps * 0.05, 0.20)  ← bonus for thorough diagnosis
