...
class PipelineWatchdogEnv(Environment):
    ...
    def __init__(self):
        ...
        self._current_event     = None  # type: Optional[dict]
    ...
    def grade(self, trajectory=None, **kwargs):
        """
        Compute final episode score strictly in (0.01, 0.99).
        ...
        try:
            ...
            base    = resolved / total
            penalty = missed * 0.10
            raw     = base - penalty + evidence
            return round(max(0.01, min(0.99, raw)), 3)
        except Exception:
            return 0.5  # safe fallback — always strictly in (0, 1)
